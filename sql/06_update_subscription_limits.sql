...
-- This script implements the pricing model matching the landing page
...
DECLARE
    ...
    v_limit_cards integer := 65;     -- Free tier: 65 Cards
    v_limit_projects integer := 2;   -- Free tier: 2 Project Areas
    v_limit_folders integer := 4;    -- Free tier: 4 Folders
BEGIN
    ...
    -- If Pro or Lifetime, allow unlimited
    IF v_tier IN ('pro_monthly', 'pro_annual', 'lifetime') THEN
        RETURN NEW;
    END IF;

    -- Check limits for ITEMS table (Free tier only)
    IF TG_TABLE_NAME = 'items' THEN
        ...
        IF NEW.type = 'project' THEN
            ...
            IF v_usage_count >= v_limit_projects THEN
                RAISE EXCEPTION 'Usage limit exceeded: Free tier is limited to % Project Areas. Upgrade to Pro for Nested Projects.', v_limit_projects;
            END IF;
        ELSE
            -- Normal card/item
            SELECT count(*) INTO v_usage_count FROM public.items 
            ...
            IF v_usage_count >= v_limit_cards THEN
                RAISE EXCEPTION 'Usage limit exceeded: Free tier is limited to % Cards. Upgrade to Pro for Unlimited Items.', v_limit_cards;
            END IF;
        END IF;
    END IF;

    -- Check limits for FOLDERS table (Free tier only)
    IF TG_TABLE_NAME = 'folders' THEN
        ...
        IF v_usage_count >= v_limit_folders THEN
            RAISE EXCEPTION 'Usage limit exceeded: Free tier is limited to % Folders. Upgrade to Pro for unlimited folders.', v_limit_folders;
        END IF;
    END IF;
...
-- 7. Pricing Details Summary (for reference)
-- FREE TIER:
--   - 65 Cards
--   - 4 Folders
--   - 2 Project Areas
...
--   - Standard Sync
--
-- PRO TIER (Monthly: $12/mo, Yearly: $10/mo):
--   - Unlimited Items
